...
app.secret_key = 'HYDRA_ULTRA_SECRET_2026'
CORS(app)
...
def register():
    ...
    if User.query.filter_by(username=data['username']).first():
        return jsonify({"status": "error", "message": "Этот логин уже занят!"})
    new_user = User(username=data['username'], password=data['password'])
    db.session.add(new_user); db.session.commit()
    return jsonify({"status": "success", "message": "Регистрация успешна! Теперь войдите."})
...
def get_nick():
    ...
    platforms = [
        ...
        {"name": "Twitter", "url": f"https://twitter.com/{n}"},
...
def get_mail():
    ...
    links = [
        {"name": "WhatsApp", "url": f"https://wa.me/{p}"},
        {"name": "Telegram", "url": f"https://t.me/+{p}"},
        {"name": "Viber", "url": f"viber://add?number={p}"},
        {"name": "GetContact", "url": f"https://www.getcontact.com/ru/search?q={p}"},
        {"name": "TrueCaller", "url": f"https://www.truecaller.com/search/int/{p}"},
        {"name": "Sync.ME", "url": f"https://sync.me/search/?number={p}"},
        {"name": "Kaspi", "url": f"https://kaspi.kz/pay/Phone?number={p}"},
        {"name": "Facebook", "url": f"https://www.facebook.com/search/top/?q={p}"}
    ]
...
def photo_scan():
    ...
    try:
        ...
        return jsonify({"status": "fail", "msg": "GPS-метки не найдены."})
